...
**Se aplica a:**
1. **`consultaOT`** (`data.js`) — filtra por el campo `campania` exacto. Afecta a **todo** lo que depende de las OT: KPIs, Avance por Cultivo, Control de Hectáreas, Alertas, Problemas Detectados, Servicios, y Auditoría (todo se construye a partir de `OTS`/`rows`).
...
- **Stock dinámico**: como un mismo Tipo de Insumo puede mezclar unidades incompatibles entre sí (Litros, Kilos, Unidades, Dosis...), el flujo Stock Inicial → Ingreso → Consumo → Balance se calcula por separado para cada combinación **(Tipo, Insumo, Unidad)** — es la unidad mínima donde sumar/restar tiene sentido. Los 4 KPIs (Stock Inicial, Ingreso, Consumo, Balance) totalizan esas filas según el filtro activo.
- **Unidad de medida en los KPIs**: la unidad que acompaña a los 4 KPIs (`unidadUnicaDe()`/`fmtKpiUnidad()` en `utils.js`) sale siempre de la columna "Unidad de medida" de `consultaInsumos` — nunca se escribe a mano ni sale de una lista fija. Se muestra **únicamente cuando `#iinsumo` tiene un Insumo puntual seleccionado** (no "Todos los Insumos"); con un Insumo elegido y una sola unidad real en sus datos (comparada sin acentos/mayúsculas/espacios, texto original al mostrar) el KPI muestra `valor Unidad` (ej. `1.250,00 Litros`). Con "Todos los Insumos" —o un Tipo de Insumo elegido pero Insumo en "Todos"— los KPIs muestran **solo el número**, sin unidad y sin ningún texto explicativo, sin importar si ese conjunto resulta tener una sola unidad o varias: la condición depende del filtro elegido, no de si el resultado es homogéneo. Si el Insumo puntual elegido igual tuviera más de una unidad en los datos (caso raro), tampoco se inventa ni se avisa: se muestra el valor sin unidad. El cálculo interno (`flujoRows`) no cambia en ningún caso, solo qué se muestra.
...
- **Gastos** (trabajos medidos en Horas o Litros, no en Unidades): el nombre de "Trabajo" en esta tabla es siempre el Servicio real de la OT (nunca una descripción inventada); cuando la OT no trae Servicio cargado, esas filas se agrupan aparte como "(Sin Servicio)". Incluye `Construccion de Puentes retro excavadora x Hs` (distinto de los dos Servicio de Puentes por Unidad) y "Desalijo" (no está en el presupuesto — se buscó "desalijo" en Servicio y Observación, excluyendo las filas cuyo Servicio ya se cuenta en otra sección para no duplicar horas/costo), separado en **2 grupos** con rótulo propio en la tabla: "Desalijo Silo Bolsa" (descarga de silo bolsa de arroz) y "Desalijo Karanda'y / Carandai" (despeje de palmera) — cada uno con una fila por Servicio real distinto encontrado, más "(Sin Servicio)" al final si aplica.
