...
1. **KPIs ejecutivos** (`#exec-kpis`, `renderResumenKPIs()`): Hectáreas Planificadas, Hectáreas Ejecutadas, Avance General (Ha Ejecutadas / Planificadas — sin techo silencioso en 100%: si se ejecuta de más se marca como posible sobre-ejecución; sin plan disponible se rotula "Sin plan" en vez de dividir por cero), Hectáreas Pendientes, OT Confirmadas, OT Atrasadas (misma definición exacta que Alertas Operacionales), Costo Ejecutado y Gasto No Agrícola. Cada tarjeta usa un acento de color a la izquierda (`.kpi-g/-y/-o/-r/-gris`) en vez de pintar toda la tarjeta.
2. **Detalle de Etapas por Cultivo** (`#cults`, `renderCultivoDetalle()`) — primer bloque analítico tras los KPIs: por cada cultivo, progreso de Preparación de Suelo/Siembra/Cuidados/Cosecha y Ha planificadas / OT confirmadas. *(Antes de esta sección hubo dos gráficos — un anillo de "Avance General de la Campaña" y una comparación por cultivo en barras horizontales — retirados a pedido del usuario; el cálculo de avance general sigue existiendo porque lo siguen usando los KPIs de arriba, solo se quitó su visualización dedicada.)*
